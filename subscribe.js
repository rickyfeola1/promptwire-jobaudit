...
export default async function handler(req, res) {
    ...
    // Unique event ID for Facebook deduplication
    const eventId = crypto.randomUUID();

    // Hash email for Facebook (SHA256 required)
    ...
    try {
        ...
        // STEP 3 — Fire Lead event to Facebook Conversions API
        if (FB_PIXEL_ID && FB_ACCESS_TOKEN) {
            try {
                ...
                if (!fbResponse.ok) {
                    ...
                    console.log('Facebook CAPI Lead fired:', fbData);
                }
            } catch (fbErr) {
        ...
        // Return event_id for browser pixel deduplication
